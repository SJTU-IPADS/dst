...
//    *pv = pie_gra_.find_or_insert(pi);
    *tv = txn_gra_.find_or_insert(pi.txn_id_,pi.timestamp_);
}
...
void DepGraph::foi_txn(uint64_t &txn_id,uint64_t &ts,
        Vertex<TxnInfo> **tv) {
    *tv = txn_gra_.find_or_insert(txn_id,ts);
}

Vertex<TxnInfo> *DepGraph::start_pie_txn(uint64_t tid,uint64_t ts) {
    Vertex<TxnInfo> *tv;
    foi_txn(tid, ts,&tv);
...
//    }
    ...
    if (tv) {
        foi_txn(pi.txn_id_, pi.timestamp_,tv);
    }
    static auto id = Config::get_config()->get_site_id();
    (*tv)->data_.servers_.insert(id);
}
