...
void RCCDTxn::start(
    ...
    uint64_t timestamp = std::max(server_read_ts,static_cast<uint64_t>(header.timestamp));

    PieInfo pi;
    pi.pie_id_ = header.pid;
    //#if PDI
    /**
     * we cannot directly use timestamp as the txn_id.
     * this is because rococo use this id for graph operations
     */
    //pi.txn_id_ = timestamp;
    //#else
    pi.txn_id_ = header.tid;
    pi.timestamp_ = timestamp;
    //#endif
    //pi.txn_id_  = new_header.timestamp;
...
void RCCDTxn::commit(
    ...
    server_read_ts = std::max(server_read_ts,static_cast<uint64_t>(req.timestamp));
