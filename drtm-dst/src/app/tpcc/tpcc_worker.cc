...
#define RPC_CUST (MAX_RPC_REGISTERED - 1)
      ...
      warehouse_id_end_(warehouse_id_end)
{
  ...
  if (worker_id_ == 1) {
    LOG(4) << "use warehouse range: " << warehouse_id_start_ << " -> " << warehouse_id_end_;
  }
...
void TpccWorker::register_callbacks() {
  ...
  if(worker_id_ == 0)  {
    LOG(3) << "TPCC register one shot callback: " << TX_STOCK_LEVEL;
  }
  one_shot_callbacks[TX_STOCK_LEVEL] = bind(&TpccWorker::stock_level_piece,this,_1,_2,_3,_4);

  rpc_->register_callback(bind(&TpccWorker::handler_customer,this,_1,_2,_3,_4), \
                          RPC_CUST);
}
...
txn_result_t TpccWorker::txn_payment(yield_func_t &yield) {
  ...
  if (strncmp(v_c->c_credit.data(), "BC", 2) == 0) {
    ...
    if (d_id == 0) d_id = 10;
    int n = snprintf(buf, sizeof(buf), "%d %d %d %d %d %f",
                     ...
                     warehouse_id,
                     paymentAmount);
    // FIXME: currently I disabled the data to reduce memory pressure
                     //v_c->c_data.c_str());
...
txn_result_t TpccWorker::txn_stock_level(yield_func_t &yield) {
  ...
  //const uint warehouse_id = PickWarehouseId(random_generator[cor_id_], warehouse_id_start_, warehouse_id_end_);
  //const uint warehouse_id = PickWarehouseId(random_generator[cor_id_], warehouse_id_start_, warehouse_id_end_);
  const uint warehouse_id = PickWarehouseId(random_generator[cor_id_],scale_factor * current_partition + 1,
                                            scale_factor * (current_partition + 1));
  ...
  //LOG(3) << "get cur next o_id: " << cur_next_o_id; sleep(1);
  ASSERT(cur_next_o_id < 99999) << cur_next_o_id;
#endif
...
txn_result_t TpccWorker::txn_super_stock_level(yield_func_t &yield) {
  ...
  if(num_remote > 0) {
    ...
    for(uint i = 0;i < num_server;++i) {
      ...
      if(!reply_p[i]) {
        //ASSERT(false) << "cannot abort";
        //LOG(4) << "check reply for " << i;
        //assert(false);
...
txn_result_t TpccWorker::txn_order_status(yield_func_t &yield) {
  ...
  if (iter.Valid() && iter.Key() >= end) {
    ...
    if(unlikely(od_seq == 1)) {
      o_id = -1; // This order is not valid, possible from a stale value
    }
...
struct CustomerReqArg {
  uint64_t c_key = 0;
  uint8_t lastname_buf[TpccMixin::CustomerLastNameMaxSize + 1];
  uint16_t warehouse_id;
  uint8_t  district_id;
};

inline uint64_t core_handle_cust(const CustomerReqArg &arg,TXHandler *tx) {
  ...
  if(arg.c_key != 0)
    ...
    std::string clast;
    clast.assign((const char *) arg.lastname_buf, 16);
    uint64_t c_start = makeCustomerIndex(arg.warehouse_id, arg.district_id, clast, zeros);
    uint64_t c_end   = makeCustomerIndex(arg.warehouse_id, arg.district_id, clast, ones);

#ifdef  RAD_TX
    RadIterator  iter((DBRad *)tx, CUST_INDEX, false);
#elif defined(OCC_TX)
    DBTXTempIterator iter((DBTX *)tx, CUST_INDEX,false);
#elif defined(FARM)
    DBFarmIterator iter((DBFarm *)tx,CUST_INDEX,false);
#elif defined(SI_TX)
    SIIterator iter((DBSI *)tx,CUST_INDEX,false);
#endif
...
txn_result_t TpccWorker::txn_payment_naive1(yield_func_t &yield) {
  ...
  if(likely((NumWarehouses() == 1) || ((RandomNumber(random_generator[cor_id_],1,100)) <= 85))) {
    ...
    customerDistrictID = RandomNumber(random_generator[cor_id_],1,NumDistrictsPerWarehouse());
    do  {
      customerWarehouseID = PickWarehouseId(random_generator[cor_id_],1,NumWarehouses() + 1);

    } while(customerWarehouseID == warehouse_id);
  }
  //LOG(3) << customerWarehouseID << " " << customerDistrictID << "; from: " << 1 << " " << NumWarehouses() + 1;
  d_key = makeDistrictKey(warehouse_id, districtID);
  ...
  char req_buf[64]; memset(req_buf,0,64);
  static_assert(sizeof(CustomerReqArg) <= 48,"xx");

  CustomerReqArg *req = (CustomerReqArg *)(req_buf + rpc_->rpc_padding());
  req->warehouse_id   = customerWarehouseID;
  req->district_id    = customerDistrictID;

  if(RandomNumber(random_generator[cor_id_], 1, 100) <= 60) {
    // should scan
    req->c_key = 0;
    GetNonUniformCustomerLastNameRun(req->lastname_buf, random_generator[cor_id_]);
  } else {
    const uint customerID = GetCustomerId(random_generator[cor_id_]);
    req->c_key = makeCustomerKey(customerWarehouseID,customerDistrictID,customerID);
  }

  // now we fetch the customer
  customer::value *v_c = nullptr;
  int pid = WarehouseToPartition(customerWarehouseID);

  uint64_t c_key;
  if(pid != current_partition) {
    // send the request
    tx_->add_to_remote_set(CUST,0,WarehouseToPartition(customerWarehouseID));
    // here is the trick
    rpc_->prepare_multi_req(tx_->remoteset->reply_buf_,1,cor_id_);
    rpc_->append_req(req_buf + rpc_->rpc_padding(),RPC_CUST,sizeof(CustomerReqArg),cor_id_,RRpc::REQ,pid);
    tx_->remoteset->read_items_ = 1;
    indirect_yield(yield);
    tx_->get_remote_results(1);
    uint64_t seq = tx_->get_cached(0,(char **)(&v_c));
    ASSERT(seq != 1);
  } else {
    c_key = core_handle_cust(*req,tx_);
    uint64_t res = tx_->get(CUST, c_key, (char **)(&v_c),sizeof(customer::value));
    ASSERT(res != 1);
  }
  ...
  if (pid == current_partition)
    ...
    tx_->remote_write(0,(char *)v_c,sizeof(customer::value));
  }

  // now update district and warehouse
  warehouse::value *v_w = nullptr;
  district::value *v_d  = nullptr;
...
#if 0 // FIXME: currently we disable the insertions, since we donot know the remote c_key
  history::value v_h;
  v_h.h_amount = paymentAmount;
  v_h.h_data.resize_junk(v_h.h_data.max_size());
  int n = snprintf((char *) v_h.h_data.data(), v_h.h_data.max_size() + 1,
...
void TpccWorker::handler_customer(int id,int cid, char *input, void *arg) {

  CustomerReqArg *c = (CustomerReqArg *)input;
  auto c_key = core_handle_cust(*c,tx_);

  // return the customer value
  char *reply_msg = rpc_->get_reply_buf(id);
  ...
  MemNode *node = store_->stores_[CUST]->Get(c_key);
  ASSERT(node != nullptr && node->value != nullptr) << "get a null customer: " << c_key;

  struct RemoteSet::ReplyHeader *r_header = (struct RemoteSet::ReplyHeader *)reply_msg;
  r_header->num_items_ = 1;
  r_header->payload_   = sizeof(struct RemoteSet::ReplyHeader) + sizeof(RemoteSet::RemoteSetReplyItem) + sizeof(customer::value);
  r_header->partition_id_ = current_partition;

retry: // coordinate concurrent modifications
  {
    auto seq = node->seq;
    asm volatile("" ::: "memory");
    ...
    if(seq == 1 || node->seq != seq)
      goto retry;
    memcpy(reply_msg_t + sizeof(RemoteSet::RemoteSetReplyItem),(char *)tmp_val + META_LENGTH, sizeof(customer::value));
    reply_item->seq = seq;
