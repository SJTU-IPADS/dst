...
workload_desc_vec_t TpceWorker::get_workload() const {
  auto res =  _get_workload();
  if (worker_id_ == 0 && cor_id_ == 1) {
    for(auto &w : res) {
      LOG(2) << "workload " << w.name << "; ratio: " << w.frequency << " " << res.size();
    }
  }
  return res;
}
...
TpceWorker::txn_trade_order(yield_func_t &yield) {
  ...
  if(!tt->tt_is_mrkt) {
    ...
    if(companyToPartition(co_id) == current_partition) {
      ...
      //LOG(3) << "remote write tr_idx: " << tr_idx;
  ...
  if(pid != current_partition) {
    ...
    //fprintf(stdout,"remote insert idx %d\n",st_idx);
...
#if 1
      if (sizeof(TTickerEntry) < 64) {
        char msg[64];
        memcpy(msg + rpc_->rpc_padding(),(char *)(&entry),sizeof(TTickerEntry));
        rpc_->append_req(msg + rpc_->rpc_padding(),RPC_ADD,sizeof(TTickerEntry),cor_id_,RRpc::REQ,pid);
      } else {
...
txn_result_t TpceWorker::txn_broker_volume(yield_func_t &yield) {
  ...
  ((DBSI *)tx_)->copy_ts_vec((char *)(arg.ts_vec));
  //#endif
#endif
...
txn_result_t TpceWorker::txn_customer_position(yield_func_t &yield) {
  ...
  if(c_header->secs > 0) {
    ...
    ((DBSI *)tx_)->copy_ts_vec((char *)(c_header->ts_vec));
    //#endif
#endif
  ...
  if(num_servers > 0) {
    ...
    for(uint i = 0;i < c_header->secs; ++i) {
      ...
      ASSERT(reply[i].val != 0) << "get value failed: " << i << "; total: " << c_header->secs;
      if(acct_idx >= acct_len) {
        //	    fprintf(stdout,"idx %d, total %d, time %lu\n",i,c_header->secs,c_header->time);
        assert(false);
      }
      ASSERT(remote_hs[acct_idx].size() > pos_idx) << "get wront index: " << pos_idx
                                                   << "; original total_size: " << remote_hs[acct_idx].size();
...
txn_result_t TpceWorker::txn_market_watch(yield_func_t &yield) {
  ...
  if(input.c_id) {
    ...
    for(uint i = 0;i < list.size();++i) {
      ...
      if(pid == current_partition) {
        ...
        /* add to forkset */
        ASSERT(pid < total_partition) << "get wrong pid: " << pid;
        fork_handler.add(pid);
  ...
  if(m_header->secs > 0) {
    ...
    ((DBSI *)tx_)->copy_ts_vec((char *)(m_header->ts_vec));
    //#endif
#endif
...
txn_result_t TpceWorker::txn_security_detail(yield_func_t &yield) {
  ...
  if(pid != current_partition) {
    ...
    ((DBSI *)tx_)->copy_ts_vec((char *)(r_input.ts_vec));
    //#endif
#endif
    //fork_handler.add(pid);
  ...
  if(pid != current_partition) {
    ...
    if (1) {
      rpc_->prepare_multi_req(reply_buf,1,cor_id_);
      rpc_->append_req(req_buf,RPC_R_VALIDATE,sizeof(SEDInput),cor_id_,RRpc::REQ,pid);
...
txn_result_t TpceWorker::trade_lookup_frame3(yield_func_t &yield, TTradeLookupTxnInput &input) {
  ...
  if(pid != current_partition) {
    ...
    ((DBSI *)tx_)->copy_ts_vec((char *)(t_input.ts_vec));
    //#endif
#endif
    fork_handler.add(pid);
  ...
  //Fork
  fork_handler.reset();
  //ForkSet fork_handler2(rpc_,cor_id_);

  // TODO: use another fork handler
  ...
  if(local_trades.size() < second_input->count) {
    ...
    ((DBSI *)tx_)->copy_ts_vec((char *)(second_input->ts_vec));
    //#endif
#endif
