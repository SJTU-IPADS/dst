...
void TpceWorker::customer_position_piece(yield_func_t &yield,int id, int cid,char *input) {
  ...
  //      assert(false);
  CPOutput *reply_msg  = (CPOutput *)(rpc_->get_reply_buf(id));
...
void TpceWorker::trade_lookup_piece0(yield_func_t &yield,int id, int cid,char *input) {
  ...
  // FIXME: seems the results are not needed. currently I send a dummy result for debug
  //rpc_->send_reply((char *)reply_msg,sizeof(uint64_t), id,worker_id_,cid);
}
