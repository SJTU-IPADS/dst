...
class MicroMainRunner : public BenchRunner {
  public:
	...
	virtual void init_store(MemDB* &store) { store = new MemDB(store_buffer); assert(store_ != NULL);
		LOG(3) << "add schema";
		using namespace nocc::oltp::bank;
		//store_->AddSchema(TAB,TAB_HASH,sizeof(uint64_t),CACHE_LINE_SZ,META_SIZE,NumAccounts() / total_partition * 1);
	}
...
void MicroWorker::thread_local_init() {
	...
	//us_per_cycle = ::nocc::util::BreakdownTimer::get_one_usec_cycles();
...
workload_desc_vec_t MicroWorker::_get_workload() {
	...
	switch (micro_type) {
		...
		case MICRO_TX_READ:{
			...
			w.push_back(workload_desc("TX rw test",(100) / 100.0,MicroTXRW));
			return w;
		}
			break;
		default:
...
#if SI_TX
	// add ts worker
	int master_id = total_partition - 1;
	ts_manager = new TSManager(nthreads,cm,master_id,0);
