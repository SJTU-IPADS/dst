...
class DBSI : public TXHandler {
  public:
    ...
    void update_ts_handler(int id,int cid,char *msg,void *arg);
    ...
    */
    ...
    void copy_ts_vec(char *dst) {
        memcpy(dst, (char *)ts_buffer_,ts_manager->ts_size_);
    }
    uint64_t get_commit_ts(yield_func_t &yield,uint64_t &encoded_commit_ts);
    void commit_ts(uint64_t ts,yield_func_t &yield); // write ts back
  ...
  private:
    ...
    int batch_num_ = 0;
    uint64_t last_ts_ = 0; // last timestamp acquired from the timestamp oracle
    uint64_t  current_ts_ = std::numeric_limits<uint64_t>::max(); // current in-used GTS
};
