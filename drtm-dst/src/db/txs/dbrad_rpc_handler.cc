...
DBRad::get_rpc_handler(int id,int cid,char *msg,void *arg) {
  ...
  for(uint i = 0;i < num_items;++i) {
    ...
    case REQ_INSERT: {
      ...
      assert(false);
#endif
      //LOG(4) << "insert main index " << (int)(header->tableid) << " " << node;
    ...
    case REQ_INSERT_IDX: {
      ...
      assert(false);
#endif
      //      LOG(4) << "insert index " << (int)(header->tableid) << " " << node;
      reply_item->seq = node->seq;
      reply_item->node = node;
      reply_item->idx  = i;
      reply_item->payload = 0;
...
DBRad::commit_rpc_handler2(int id,int cid,char *msg,void *arg) {
  ...
  for(uint i = 0;i < num_items;++i) {
    ...
    //LOG(4) << "update table " << (int)header->tableid << " ; pid " << (int)header->pid
    //<< "; payload " << header->payload << "; node " << header->node;
    if(header->pid != current_partition ) {
    ...
    //LOG(4) << "oldptr " << oldptr << ", update cure "<< cur;
    if(cur != NULL) {
