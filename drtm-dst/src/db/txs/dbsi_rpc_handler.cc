...
void DBSI::commit_rpc_handler2(int id,int cid,char *msg,void *arg) {
  ...
  for(uint i = 0;i < num_items;++i) {
    ...
    if(header->node->lock != ENCODE_LOCK_CONTENT(id,thread_id,cid + 1))
      continue;
...
void DBSI::acquire_ts_handler(int id,int cid,char *msg,void *arg) {
  ...
  char *reply_msg = rpc_->get_reply_buf(id);
#if !DUMMY_FUNC
  uint64_t res = ts_manager->get_gts();
  //LOG(4) << "get res " << res;
...
void DBSI::update_ts_handler(int id,int cid,char *msg,void *arg) {
  ts_manager->ts_update_handler(id,cid,msg,arg);
  rpc_->send_null_reply(id,cid);
}
