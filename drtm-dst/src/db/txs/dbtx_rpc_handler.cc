...
void DBTX::get_naive_rpc_handler(int id,int cid,char *msg,void *arg) {
  ...
  char *reply_buf = rpc_->get_reply_buf(id);
...
void DBTX::get_rpc_handler(int id,int cid,char *msg,void *arg) {
  ...
  /* send reply */
  //LOG(3) << num_item_fetched << " returned, " << num_items << " requested";
...
void DBTX::commit_rpc_handler2(int id,int cid,char *msg,void *arg) {
  ...
  for(uint i = 0;i < num_items;++i) {
    ...
    MemNode *node = (MemNode *)(header->node);
    ASSERT(node != nullptr) << "get null node for key: " << (uint64_t)(header->key) << ",@table " << (int)header->tableid;
    if(header->payload == 0) {
      /* a delete case */
      new_val = NULL;
    } else {
