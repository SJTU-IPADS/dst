...
uint64_t DBTX::get_ro(int tableid,uint64_t key,char *val,yield_func_t &yield) {
  ...
  uint64_t tentative_timestamp = MAX(timestamp,origin);
  ...
  uint64_t seq = node->seq;
  asm volatile("" ::: "memory");
  if(unlikely(node->lock != 0 && seq < timestamp)) {
...
uint64_t DBTX::get(int tableid, uint64_t key, char **val,int len) {
  ...
  ASSERT(node != NULL && node->value != NULL)
      << "read error with node [" << node << "];"
      << "value [" << (node?node->value : (uint64_t *)(0x737373)) << "];"
      << "for table:  " << tableid;
#endif
retry:
  uint64_t seq = node->seq;
