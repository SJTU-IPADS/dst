...
#if LARGE_VEC == 1
__thread uint64_t TSManager::local_ts = 3;
#else
...
inline int DBSI::WriteSet::CommitLocalWrite(uint64_t commit_ts) {
    ...
    for(uint i = 0;i < elems;++i) {
        if(kvs[i].ro)
            continue;
#if NO_EXE_ABORT
        if (kvs[i].node->lock !=  ENCODE_LOCK_CONTENT(current_partition,tx_->thread_id,
                                                      tx_->cor_id_ + 1))
            continue;
#endif
          ...
          TXHandler(c_id) // farther class
{
...
#if TS_USE_MSG == 1
    rpc_->register_callback(std::bind(&DBSI::update_ts_handler,this,
    ...
    for(uint i = 0;i < cm->get_num_nodes();++i) {
        ...
        if (qp != nullptr)
            ...
            if(c_id == 1 && thread_id == 0)
                LOG(5) << "SI does not has RC QP!";
        }
    }
#endif
    ...
    if(thread_id == 0 && cor_id_ == 1) {
        LOG(4) << "use TS batch size as " << TS_BATCH;
    }
}
...
retry:
    ...
    asm volatile("" ::: "memory");
#if ONE_CLOCK == 0
    //LOG(3) << "ts buf entry: "<< ts_buffer_[SI_GET_SERVER(seq)] << "; get counter: " << SI_GET_COUNTER(seq); sleep(1);
...
#endif
    //LOG(4) << "commit remote ts done"; sleep(1);
#if ONE_CLOCK == 1
    this->commit_ts(commit_ts,yield); // commit the timestamp to the remote oracle
#endif
    return true;
...
uint64_t DBSI::get_commit_ts(yield_func_t &yield,uint64_t &encoded_commit_ts) {
    uint64_t commit_ts;
#if ONE_CLOCK == 0 // case for VTS, acquire locally
    //LOG(4) << "in VTS handle";
#ifdef EM_OCC
...
#else // case for a GTS, send an RPC
    //LOG(4) << "in GTS handler";
    if(current_ts_ < last_ts_) {
        // current_ts is can be used
        ASSERT(false);
        return ++current_ts_;
    }
...
#endif
    char reqs[64];
    char *req_buf = reqs + rpc_->rpc_padding();
    TSManager::UpdateArg *input = (TSManager::UpdateArg *)req_buf;
    input->thread_id = thread_id;
    input->counter   = ts;
    int master_id = ts_manager->master_id_;

    //LOG(4) << "update ts " << current_ts_; sleep(1);
    char dummy;
    rpc_->prepare_multi_req(&dummy,1,cor_id_);
