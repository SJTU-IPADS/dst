...
class RRpc {
 ...
 public:
...
#endif
  inline char *get_reply_buf(int mid = 0) {
    return reply_buf_pool_[mid][reply_buf_slots_[mid]++ % 64];
  }
  ...
  inline void send_null_reply(int server_id,int cid) {
    char *msg = get_reply_buf();
    send_reply(msg,0,server_id,cid);
  }
 ...
 private:
  ...
  std::vector<std::vector<char *> > reply_buf_pool_;
  std::vector<std::vector<char *> >req_buf_pool_;
  std::vector<uint16_t> reply_buf_slots_ ;
