...
RRpc::RRpc(int tid,int coroutines,int req_buf_num,int reply_buf_num)
    : worker_id_(tid),
      reply_buf_pool_(total_partition),
      reply_buf_slots_(total_partition,0)
{
  ...
  int reply_msg_size = 8192; assert(reply_msg_size > rpc_padding()); // we uses a fixed reply msg size
  ...
  for(uint i = 0;i < coroutines + 1;++i) {
    ...
    for(uint j = 0;j < req_buf_num;++j) {
      char *buf = (char *)Rmalloc(4096); assert(buf != NULL);
  ...
  for(uint i = 0; i < total_partition;++i) {
    for(uint j = 0;j < 64;++j) {
      char *buf = (char *)Rmalloc(reply_msg_size); assert(buf != NULL);
      reply_buf_pool_[i].push_back(buf);
    }
  }
  ...
  if(header->meta.type == REQ) {
    ...
    if(unlikely(reply_counts_[header->meta.cid] <= 0)) {
      LOG(7) << "receive a reply from cid" << header->meta.cid << " at worker " << worker_id_ << " @mac " << from
             ...
             << "; msg size: " << (int)header->meta.payload;
    }
    //LOG(3) << "recevei a reply of size: " << (int)header->meta.payload;
