...
#if SI_TX
  //  db::TSManager *ts = (db::TSManager *)((*workers_)[nthreads]);
  //ts->report();
#endif
  // calculate latency
  auto latency = 0;
...
double BenchReporter::end() {
  LOG(4) << "total " << thpts.size() << " points recorded.";
  if(thpts.size() == 0) return 0.0;
  double sum = 0;int c(0);
  std::sort(thpts.begin(),thpts.end());
#if 1
  std::vector<int> report_res;
  for(int i = 4;i < (int)(thpts.size()) - 1;++i) {
    report_res.push_back(thpts[i]);
    sum += thpts[i];c++;
  }
  for(auto k : report_res)
    LOG(3) << "report " << k;
#endif
