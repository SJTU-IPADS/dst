...
class BenchWorker : public RWorker {
 public:
  ...
  struct REQ {
    ...
    uint64_t start_time;
    uint64_t padding;
    REQ(int t,int cid,int ctid,int cor,uint64_t s = 0) : tx_id(t),c_id(cid),c_tid(ctid),cor_id(cor),
                                                         start_time(s)
    {}
    REQ() { }
  };
...
class BenchClient : public RWorker {
 public:
  ...
  virtual void run();
  virtual void worker_routine(yield_func_t &yield);
  void worker_routine_local();
  void worker_routine_local2();

  bool send_one(uint64_t res = 0);
  ...
  std::queue<BenchWorker::REQ> pending_reqs;
};
