...
BenchRunner::run() {
  ...
  LOG(4) << "check flags: rdma (" << USE_RDMA
         << "); USE_AWS (" << USE_AWS
         << "); USE_TCP_MSG ( " << USE_TCP_MSG << ")";
#if USE_UD_MSG == 0 && USE_TCP_MSG == 0
  LOG(3) << "use RDMA ring message";
  // Calculating message size
  //using namespace rdmaio::ring_imm_msg;
  using namespace rdmaio::ringmsg;
  ring_padding = MAX_MSG_SIZE;
  total_ring_sz = 200 * (2 * MAX_MSG_SIZE + 2 * 4096)  + ring_padding + MSG_META_SZ; // used for applications
  ...
  poller = new AdapterPoller(nthreads + nclients + 1,local_comm_queues,tcp_port);
...
void BenchRunner::parse_config(std::string &config_file) {
  ...
  LOG(2) << "use scale factor: " << scale_factor << "; with total " << nthreads << " threads;" << "coroutines: " << coroutine_num;
