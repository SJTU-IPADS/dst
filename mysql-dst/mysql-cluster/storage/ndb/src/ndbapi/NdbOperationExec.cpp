...
class OldNdbApiSectionIterator: public GenericSectionIterator
{
 private :
  ...
  void checkStaticAssertions()
  {
    ...
    //STATIC_ASSERT(AttrInfo::HeaderLength == KeyAndAttrInfoHeaderLength + 2);
  };
...
NdbOperation::prepareSend(Uint32 aTC_ConnectPtr, 
                          Uint64 aTransId,
                          AbortOption ao,Uint64 timestamp)
{
  ...
  //RDMA_LOG(3) << "prepare send tid: " << aTransId << "; ts: " << timestamp;
  theErrorLine = 0;
  ...
  Uint32 ts1 = (Uint32) timestamp;
  Uint32 ts2 = (Uint32) (timestamp >> 32);
  //RDMA_LOG(3) << "send two timestamp :" << ts1 << " " << ts2;
  //RDMA_LOG(3) << "send two TX ID: " << tTransId1 << " " << tTransId2;
