...
#if MVCC
  static std::map<Uint32* , TimeTravel*> timestamp_table;
#endif
...
void Dbtup::prepareTUPKEYREQ(Uint32 page_id,
                             ...
                             Uint32 frag_id)
{
  //<< prepare_tuple_ptr; // XD: its not read, its a prefetch
  FragrecordPtr fragptr;
  TablerecPtr tabptr;
  ...
  RDMA_LOG(3) << "record MVCC here";
#if MVCC
  // XD: record the multi-version here
  if (timestamp_table.find((Uint32 *)(prepare_tuple_ptr)) == timestamp_table.end()) {
    TimeTravel *timeTravel = new TimeTravel();
    RDMA_LOG(3) << "time travel timestamp: " << timeTravel->timestamp;
    timeTravel->old_ptr = NULL;
    timeTravel->timestamp = 0;
    timestamp_table.insert(std::make_pair((Uint32 *)(prepare_tuple_ptr), timeTravel));
...
/**
 * XD: seems in here to fetch the key
 */
bool Dbtup::execTUPKEYREQ(Signal* signal) 
{
   RDMA_LOG(1) << "execute tupkey req";
   TupKeyReq * tupKeyReq= (TupKeyReq *)signal->getDataPtr();
   Ptr<Operationrec> operPtr = prepare_oper_ptr;
   KeyReqStruct req_struct(this);
   RDMA_LOG(4) << "get ts: " << tupKeyReq->ts1 << " " << tupKeyReq->ts2;
   jamEntryDebug();
   Uint64 timestamp = ((Uint64)tupKeyReq->ts2 << 32) | tupKeyReq->ts1;
 ...
 /* ----------------------------------------------------------------- */
   ...
   if (likely(Roptype == ZREAD))
   {
     ...
     if (unlikely(isCopyTuple(pageid, pageidx)))
     {
       ...
       //RDMA_LOG(1) << "reads a copy-tuple directly";
       setup_lcp_read_copy_tuple(&req_struct, regOperPtr, regTabPtr);
     }
     else
     {
       //RDMA_LOG(1) << "get tuple pointer " << req_struct.m_tuple_ptr;
       /**
       ...
       //       RDMA_LOG(1) << "before setup read " << req_struct.m_tuple_ptr;
     ...
     RDMA_LOG(1) << "before handle read, get tuple pointer " << req_struct.m_tuple_ptr;
     // uses this m_tuple_ptr to find the history value of this record
#if MVCC
     Tuple_header *base= req_struct.m_tuple_ptr;
     if(base->timestamp > timestamp) {
       std::map<Uint32*, TimeTravel*>::iterator it = timestamp_table.find(prepare_tuple_ptr);
       if(it == timestamp_table.end()) {
       } else {
         TimeTravel *t = it->second;
         while(t->old_ptr != NULL) {
           if(t->timestamp < timestamp) {
             RDMA_LOG(3) << "Read old value with ts: " << t->timestamp;
             req_struct.m_tuple_ptr = (Tuple_header *)(t->data);
             break;
           }
           t = t->old_ptr;
         }
   ...
   do_insert:
     ...
     if (Roptype == ZUPDATE) {
       jamDebug();
       RDMA_LOG(2) << "call handleUpdateReq()";
#if MVCC
       RDMA_LOG(4) << "handle MVCC updates";
       //std::map<Uint32*, TimeTravel*>::iterator it = timestamp_table.find(prepare_tuple_ptr);
       std::map<Uint32*, TimeTravel*>::iterator it = timestamp_table.find(prepare_tuple_ptr);
       assert(it != timestamp_table.end());
       //Tuple_header *header  = (Tuple_header *)prepare_tuple_
       //new_entry->timestamp =
       //assert(regTabPtr->total_rec_size + Tuple_header::HeaderSize < 128);
       if(regTabPtr->total_rec_size + Tuple_header::HeaderSize < 128) {
         TimeTravel *new_entry = new TimeTravel;
         RDMA_LOG(4) << "MVCC update real update <" << prepare_tuple_ptr << ">";
         Tuple_header *base= req_struct.m_tuple_ptr;
         memcpy(new_entry->data,req_struct.m_tuple_ptr,regTabPtr->total_rec_size + Tuple_header::HeaderSize);
         new_entry->old_ptr = it->second;
         new_entry->timestamp = base->timestamp;
         timestamp_table[prepare_tuple_ptr] = new_entry;
       } else {
         RDMA_LOG(4) << "ignore large size: " << regTabPtr->total_rec_size + Tuple_header::HeaderSize;
       }
           //memcpy(it->second->data, req_struct.m_tuple_ptr, regTabPtr->total_rec_size + Tuple_header::HeaderSize);
#endif
...
int Dbtup::handleReadReq(Signal* signal,
                         ...
                         KeyReqStruct* req_struct)
{
  RDMA_LOG(1) << "handle read request ";
  /**
   * Code for handing read request
   */
  //  assert(false);
  Uint32 *dst;
...
/* ------------------------------------------------------------------------- */
      ...
      RDMA_LOG(1) << "num of data read " << ret;
...
int Dbtup::handleUpdateReq(Signal* signal,
			   ...
			   bool disk) 
{
  RDMA_LOG(3) << "handle update request";
  Tuple_header *dst;
  Tuple_header *base= req_struct->m_tuple_ptr, *org;
  ChangeMask * change_mask_ptr;
  RDMA_LOG(3) << " rec size: " << regTabPtr->total_rec_size;
...
{
  RDMA_LOG(3) << "handle insert request";
