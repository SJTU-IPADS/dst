...
#define PRINT_ERROR(code,msg)                                       \
	printf(" Error in %s : %d , code: %d msg: %s", __FILE__, __LINE__, code, msg);

  /* std::cout << "Error in " << __FILE__ << ", line: " << __LINE__    \ */
  /* << ",unsign int code: " << code                                             \ */
  /* << ", const char * msg: " << msg << "." << std::endl */
	...
	char           *argv[];
{
  ...
  /* Parse args */
	...
	/* resp = mysql_real_connect(mysql, "192.168.15.108", "root", "123", "", 0,"/home/wxd/mysql-base/mysql.sock", 0); */
        /* if (!resp) */
            /* MYSQLERROR(*mysql); */

	resp = mysql_real_connect(mysql, connect_string, db_user, db_password, db_string, 3306, socket, 0);
        if (!resp)
	        MYSQLERROR(*mysql);
	...
	if( mysql_stmt_prepare(stmt[10],
			       ...
			       50) ) goto Error_SqlCall_close;
	if( mysql_stmt_prepare(stmt[11],
			       "INSERT INTO orders_index values(?,?)",
			       36) ) goto Error_SqlCall_close;
	if( mysql_stmt_prepare(stmt[12],
			       "INSERT INTO new_orders_index values(?)",
			       38) ) goto Error_SqlCall_close;
...
retry:
    if (retried)
        printf("Retrying ...\n");
    retried = 1;
  for (int roundNum = 0; roundNum < 10; roundNum++) {
	for (i_id = (roundNum * MAXITEMS / 10 + 1); i_id <= (roundNum+1)*MAXITEMS/10; i_id++) {
...
retry:
  for (int roundNum = 0; roundNum < 10; roundNum++) {
	for (s_i_id = (roundNum * MAXITEMS / 10 + 1); s_i_id <= (roundNum+1)*MAXITEMS/10; s_i_id++) {
...
retry:
	...
	for (o_id = 1; o_id <= ORD_PER_DIST; o_id++) {
        ...
        // insert into orders_index table
        int c_d_w = (o_c_id-1) + (o_d_id-1) * 3000 + (o_w_id-1) * 30000;
        memset(param, 0, sizeof(MYSQL_BIND) * 2);
        param[0].buffer_type = MYSQL_TYPE_LONG;
        param[0].buffer = &c_d_w;
        param[1].buffer_type = MYSQL_TYPE_LONG;
        param[1].buffer = &o_id;
        if( mysql_stmt_bind_param(stmt[11], param) ) goto sqlerr;
        if( try_stmt_execute(stmt[11]) ) goto retry;
        ...
        // insert into new_orders_index table
        int32_t up_id = (o_w_id-1) * 10 + o_d_id-1;
        int32_t low_id = o_id - 1;
        int64_t w_d_o = ((int64_t)up_id << 32) | ((int64_t)low_id);
        /* printf("w_d_o : %" PRId64 "\t", w_d_o); */ 
        memset(param, 0, sizeof(MYSQL_BIND) * 1);
        param[0].buffer_type = MYSQL_TYPE_LONGLONG;
        param[0].buffer = &w_d_o;
        if( mysql_stmt_bind_param(stmt[12], param) ) goto sqlerr;
        if( try_stmt_execute(stmt[12]) ) goto retry;
    ...
    int orderline_carrier_data = 1;
		...
		for (ol = 1; ol <= o_ol_cnt; ol++) {
			...
			if (o_id > 2100) {
			    ...
			    param[5].buffer = &ol_supply_w_id;
			    param[6].buffer_type = MYSQL_TYPE_LONG;
			    param[6].buffer = &orderline_carrier_data;
