...
// XD: we check that DST optimization is enabled
//BOOST_STATIC_ASSERT((DST == 1) && "Must have 64-bit long!")
...
STATIC_ASSERT((DST == 1) && "DST must be enabled");
#endif
...
void Dbacc::initOpRec(const AccKeyReq* signal, Uint32 siglen) const
{
  ...
  //RDMA_LOG(4) << "in initOpRec: ts1  "<< signal->ts1 << " " <<signal->ts2;
  /**
   * DST: recover the timestamp here
   */
  uint64_t timestamp = (uint64_t)(signal->ts2) << 32 | signal->ts1;
  //operationRecPtr.p->timestamp = (signal->ts2 << 32) | signal->ts1;
  operationRecPtr.p->timestamp = timestamp;
  RDMA_LOG(3) << "recovery timestamp : " <<     operationRecPtr.p->timestamp;
  // print
  ...
  //operationRecPtr.p->read_flag = readOp;
...
void Dbacc::execACCKEYREQ(Signal* signal)
{
...
#if DST == 1
  int dst_type = (int)(AccKeyReq::getReplicaType(req->requestInfo));
#endif
  ...
  // we first check whether it is possible to donot lock for the simple operation
  bool whether_dirty = false;
#if DST == 1
  if (lockOwnerPtr.p && (dst_type == 1)) {
    if (lockOwnerPtr.p->timestamp > operationRecPtr.p->timestamp) {
      RDMA_LOG(3) << "dst avoids locks here !";
      whether_dirty = true;
    } else {
      RDMA_LOG(3) << "dst still has to lock";
#if 0
      if(lockOwnerPtr.p->is_read_tx()) {
        // it could happen, although I donot know how
        // RDMA_ASSERT(false) << " tx ts: " << lockOwnerPtr.p->timestamp;
      } else {
        RDMA_LOG(3) << "lock owner ts: " << lockOwnerPtr.p->timestamp << " ;my ts: " << operationRecPtr.p->timestamp
                    << "; lock owner TX id: " << lockOwnerPtr.p->transId1 << " " << lockOwnerPtr.p->transId2;
      }
#endif
    }
    //whether_dirty = true;
  ...
  if (found == ZTRUE)
  {
    switch (op) {
      ...
      case ZWRITE:
      case ZSCAN_OP:
        if ((!lockOwnerPtr.p) || whether_dirty)
        {
          RDMA_LOG(3) << "no contention";
          if(op == ZWRITE)
          {
            jam();
          ...
          //RDMA_LOG(3) << "check opbits: (lock_mode)"  << (int)(opbits & Operationrec::OP_LOCK_MODE);
          int lock_mode = (int)(opbits & Operationrec::OP_LOCK_MODE);
          int dirty_read = (opbits & Operationrec::OP_DIRTY_READ);

#if DST == 1
          //RDMA_LOG(3) << "check dst_type " << dst_type << "; lock_mode: " << lock_mode;
          if ((lock_mode == 0) && (dst_type == 1)) {
            // we only change operation in : shared mode, and replication (this flag is reused as multi-version mode)

            //RDMA_LOG(5) << "DST change operation!";
            dirty_read = 1;
          }
#endif
          //if(whether_dirty)
          //ndbrequire(dirty_read);
          ...
          if (! dirty_read ) {
            ...
            RDMA_LOG(4) << "Set lock to [" << elemPageptr.p << ";" << elemptr << "]";
...
Dbacc::accIsLockedLab(Signal* signal, OperationrecPtr lockOwnerPtr) const
{
  ...
  if ((bits & Operationrec::OP_DIRTY_READ) == 0){
    ...
    if (return_result == ZPARALLEL_QUEUE) {
      ...
      sendAcckeyconf(signal);
      // TODO: we should set the whether lock success here
      return;
...
void Dbacc::execACC_COMMITREQ(Signal* signal)
{
  ...
  {
    AccKeyReq* const req = reinterpret_cast<AccKeyReq*>(&signal->theData[0]);
    int dst = (int)(AccKeyReq::getReplicaType(req->requestInfo));
    int lock_mode = (int)(opbits & Operationrec::OP_LOCK_MODE);
    int dirty_op  = (int)AccKeyReq::getDirtyOp(req->requestInfo);
    //RDMA_LOG(3) << "before check commit operation: " << dst << " " << lock_mode << " " <<dirty_op;
    dst_type = ((dst == 1) && dirty_op);
  }
  //RDMA_LOG(3) << "check commit operation type: " << (int)Toperation;
  if(dst_type) {
    RDMA_LOG(4) << "in dst op? " << (int)Toperation << " " << dst_type;
    //RDMA_ASSERT(false);
  }else{
...
void Dbacc::execACC_COMMITDST(Signal* signal)
{
  Uint8 Toperation;
  jamEntry();
  operationRecPtr.i = signal->theData[0];
...
#else
  /**
   * DST donot need to commit the operation
   */
  //commitOperation(signal);
#endif
  ndbassert(operationRecPtr.i == tmp);
...
{
  //fprintf(stdout,"!!!!!!!exe acc lockreq\n");
  jamEntry();
  ...
  if (lockOp == AccLockReq::LockShared ||
    ...
    jam();
    //fprintf(stdout,"lock the pointer for %d\n",req->tableId);
    // find table
    tabptr.i = req->tableId;
    ptrCheckGuard(tabptr, ctablesize, tabrec);
    ...
    if (tgeRemLen >= Container::HEADER_SIZE + TelemLen) {
      ...
      do {
            ...
            operationRecPtr.p->hashValue.match(fragrecptr.p->level.enlarge(reducedHashValue, bucket_number)))
        {
          ...
          {
            jamDebug();
            RDMA_LOG(4) << "get local key: "
                        << "[page num]  " << localkey.m_page_no << ";"
                        << "[page_indx] " << localkey.m_page_idx << ";"
                        << "[page_file_no] " << localkey.m_file_no;
            operationRecPtr.p->localdata = localkey;
            return ZTRUE;
          }
        }
  ...
  if (opbits & Operationrec::OP_LOCK_OWNER)
  {
    ...
    else
    {
      ...
      if ((opbits & Operationrec::OP_ELEMENT_DISAPPEARED) == 0)
      {
        ...
        //fprintf(stdout,"set unlock %d\n",operationRecPtr.p->elementPage);
        //RDMA_LOG(1) << "set unlock";
...
void Dbacc::commitOperation(Signal* signal)
{
  ...
  if (opbits & Operationrec::OP_LOCK_OWNER)
  {
    ...
    if (!queue && (opbits & Operationrec::OP_ELEMENT_DISAPPEARED) == 0)
    {
      ...
      RDMA_LOG(4) << "set unlock to [" << coPageidptr.p << ";" << tcoElementptr << "]";
      return;
    }
    else if (queue)
    {
 ...
 *
 */
void Dbacc::insertLockOwnersList(const OperationrecPtr& insOperPtr,uint64_t timestamp) const
{
  OperationrecPtr tmpOperPtr;
#ifdef VM_TRACE
