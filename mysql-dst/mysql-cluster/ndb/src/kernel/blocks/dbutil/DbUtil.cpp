...
//  Uint32 totalDataLen = headerPtr.sz + dataPtr.sz;
 ...
 /***********************************************************
  ...
  //Uint32* tempBuf = signal->theData + 27;
...
DbUtil::runOperation(Signal* signal, TransactionPtr & transPtr, 
  ...
  //RDMA_LOG(4) <<  "before send signal "<< GSN_TCKEYREQ <<" " << sigLen;
  sendSignal(transPtr.p->connectRef, GSN_TCKEYREQ, signal, sigLen, JBB);
  //RDMA_LOG(4) <<  "end send signal "<< GSN_TCKEYREQ <<" " << sigLen;
  ...
  //RDMA_LOG(4) << "send attrinfo done " << attrInfo;
  op->attrInfo.first(ait);
  sendAttrInfo(signal, transPtr.p->connectRef,
               attrInfo, op->attrInfo, ait);
  //RDMA_LOG(4) << "send attrinfo2 done";
  //  RDMA_LOG(4) << "run one op done";
}
...
#endif
    //RDMA_LOG(4) << "before send attr: " << AttrInfo::HeaderLength + i;
