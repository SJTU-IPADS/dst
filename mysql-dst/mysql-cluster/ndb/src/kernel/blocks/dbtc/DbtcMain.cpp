...
void Dbtc::execTCSEIZEREQ(Signal* signal) 
{
  ...
  //fprintf(stdout,"exec TCSEIZEREQ, from %d node\n",senderNodeId);
...
void Dbtc::execTCKEYREQ(Signal* signal) 
{
  ...
  uint32 ts1(tcKeyReq->ts1), ts2(tcKeyReq->ts2);
  ...
  //fprintf(stdout,"read table id %u\n",TtabIndex);
  ...
  // XD: DST pass the timestamp here
  regApiPtr->tss[0] = ts1; regApiPtr->tss[1] = ts2;
...
{
  /**
   * XD: it seems that its the main entry point of each request
   */
  UintR tnoOfBackup;
  UintR tnoOfStandby;
                         ...
                         BlockReference TBRef)
{
  ...
  // XD: I add this
  UintR ts1 = regApiPtr->tss[0];
  UintR ts2 = regApiPtr->tss[1];
  ...
  if (regCachePtr->useLongLqhKeyReq)
  {
    ...
    RDMA_LOG(3) << "send dbtc req with ID" << sig2 << " " << sig3;
