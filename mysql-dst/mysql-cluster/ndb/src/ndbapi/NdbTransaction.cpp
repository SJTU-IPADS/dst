...
static uint64_t get_timestamp() {
  ...
  //RDMA_LOG(4) << "TX get ts: " << (uint64_t)t;
  return (uint64_t)t;
}
...
{
  //RDMA_LOG(1) << "ndb TX get timestamp " << timestamp;
...
#define CHECK_SZ(mask, sz) assert((sizeof(mask)/sizeof(mask[0])) == sz)
  ...
  //Uint32 tTransId1 = (Uint32) theTransactionId;
  //Uint32 tTransId2 = (Uint32) (theTransactionId >> 32);
  //RDMA_LOG(3) << "alloc TX id: " <<  tTransId1 << " " << tTransId2;
                        ...
                        int forceSend)
{
  ...
  //fprintf(stdout,"ndb TX enter execute");
  ...
  //fprintf(stdout,"commit done %d",ret);
  DBUG_RETURN(ret);
}
                                     ...
                                     NdbOperation::AbortOption abortOption)
{
  ...
  while (tOp) {
    ...
    if (tOp->Status() == NdbOperation::UseNdbRecord)
      ...
      //RDMA_LOG(3) << "start prepare send";
      tReturnCode= tOp->prepareSend(theTCConPtr, theTransactionId, abortOption,timestamp);
    }
